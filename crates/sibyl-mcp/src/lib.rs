//! sibyl-mcp — thin product capsule over the shared agent broker.
...
impl SibylMcp {
    ...
    /// sibyl.v1.request.mcp.tools.list — compatibility alias.
    #[astrid::interceptor("handle_sibyl_mcp_list")]
    pub fn handle_sibyl_mcp_list(&self, payload: serde_json::Value) -> Result<(), SysError> {
        ensure_product();
        handlers::handle_mcp_list(payload)
    }
