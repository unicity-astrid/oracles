# Multiple Codex Windows With Sibyl

## Question

What happens when multiple Codex CLI windows run with the Sibyl plugin
enabled?
...
- On `SessionStart`, each window runs:
  `astrid-plugin/bin/astrid-up sibyl session-start`.
- The current shim converges every window onto one Astrid agent principal named
  `sibyl-code` by default.
- Hook events are routed through `astrid-up sibyl hook <event>`.
...
Sibyl keeps `sibyl-code` as the default plugin principal and adds a
session discriminator:

- `sibyl` records `sibyl.session.<session_id>` metadata in KV.
...
- Plugin hook events now target `sibyl.v1.hook.<session_id>.<event>`.
...
If two windows start before the `sibyl-code` principal exists, both can observe
...
2. Re-check `astrid agent show sibyl-code`.
3. Create if still missing.
4. Treat "already exists" as success.

### Identity Collapse

All windows currently share the `sibyl-code` principal. That is acceptable for a
...
- Principal: durable governed identity, for example `sibyl-code`.
...
collapse into the same `sibyl.v1.hook.*` stream and become hard to audit or
route.

Hook events should carry both routing and audit attribution:

- Topic discriminator: `sibyl.v1.hook.<session_id>.<event>`.
...
- Keep one `sibyl-code` principal by default.
...
`sibyl-code` principal. Before claiming true multi-agent management, add:
