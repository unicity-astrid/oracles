...
Installs the Sibyl Codex plugin against a matched Astrid CLI/daemon pair.
...
echo "Sibyl configured with ASTRID_BIN_ROOT=$root"
...
if [ "$skip_codex_install" = "0" ]; then
  ...
  codex plugin add sibyl@sibyl-local
fi
