...
- `sibyl` is the governed Codex runner capsule.
- `sibyl-install` is the bootstrap/relink capsule for Codex config, hooks, and
  plugin linkage.
- `sibyl-mcp` is the Codex-facing MCP/tool broker. It serves
...
- The Codex plugin `.mcp.json` enters through
  `astrid-up --principal sibyl-code`, mirroring Sage's `claude-code` principal
  shape. The shim defaults blank or unexpanded principal config to `sibyl-code`,
  boots or reuses an ephemeral Astrid daemon, provisions the scoped principal
  in the `sibyl` family group, then execs
...
- Multiple Codex windows should normally share the same durable principal
  (`sibyl-code`) and differ by `session_id`. Split principals only for real
...
- `codex exec --json` remains useful for bounded automation; its JSONL stream
  ...
  Astrid bus.
- The Sibyl runner mirrors bounded `codex exec --json` records onto
  `sibyl.v1.event.<session_id>.codex.<event-type>` topics by default.

## Codex Hook Translation Plan

- On plugin `SessionStart`, call Astrid first to ensure the configured
  principal, defaulting to `sibyl-code`, and publish a session-start event.
- Native Codex hooks should be translated to
  `sibyl.v1.hook.<session_id>.<event>` topics with the original Codex hook
...
- Current installed Astrid CLI has `agent` management but no native
  `astrid emit` or `astrid sibyl` namespace yet. The plugin `astrid-up` shim is
...
- `astrid sibyl doctor`: checks Codex CLI version, hook trust state, active
...
## Remaining Sibyl Work

- Promote the plugin `astrid-up sibyl up` compatibility launcher into a native
  `astrid sibyl up` command.
